...
public class Accounts {
	...
	public List<String> getAccountsWithProjects() {
		...
		for (int i = 2; i < numAccounts; i++) {
			...
			numProjects -= hidProj;
			// if project count is still greater than 0, then traverse all non-hidden
			// projects
			if (numProjects > 0) {
				...
				for (int k = 1; k < numProjects + 1; k++) {
					...
					if (!(project.getAttribute("class").equals("sub-project"))) {
						driver.findElement(
								By.xpath("//th[contains(text(),'Status')]/../../../tbody/tr[" + k + "]/td/a")).click();
	...
	/**
	 * Traverses all accounts and determines if they have any projects. If no
	 * projects exist, the name of the account is added to a list of Strings.
	 * 
	...
	/**
	 * Given a list of account names, this returns true if all accounts do not
	 * appear in QuickNav. Returns false otherwise.
	 * 
	 * @param names,
	...
	public boolean verifyAccountsQuickNavNoDisplay(List<String> names) {
		List<Element> quickNavAccounts = driver.findElements(By.xpath("//div[@class='quick-nav-group']/a[@class='project-row  list-group-item']"));
		List<String> quickNavAccountNames = new ArrayList<String>();
		for (Element element : quickNavAccounts) {
			quickNavAccountNames.add(element.getText());
		}
		
		for (String string : quickNavAccountNames) {
